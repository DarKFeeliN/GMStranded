...
function PANEL:Init()
	...
	local ToolTables = spawnmenu.GetTools()

	if ( !ToolTables || table.IsEmpty( ToolTables ) ) then LocalPlayer():ChatPrint( "ERROR: Tools List could not be loaded." ) return end
...
function PANEL:AddCategory( Name, Label, ToolItems )
	...
	for k, v in pairs( ToolItems ) do
		...
		Item:SetText( v.Text )
		Item.DoClick = function( button ) self:EnableControlPanel( button ) end
		concommand.Add( Format( "tool_%s", v.ItemName ), function() Item:DoClick() end )
...
function PANEL:EnableControlPanel( button )
	...
	-- This is a quick and dirty nasty hack to stop the context menu from being attached to the original spawnmenu after attaching to our custom one
	g_SpawnMenu.ToolMenu = { GetToolPanel = function() end }

	-- First run the consoel command to make the player equip the toolgun
	if ( button.Command ) then
		LocalPlayer():ConCommand( button.Command )
	end

	-- Then build the context menu
...
function PANEL:Init()
	...
	local populatearea = vgui.Create( "DLabel", self )
	populatearea:SetTall( 64 )
	populatearea:SetText( "Map and character saves are separate" )
	self.MapSaving:AddItem( populatearea )
